...
from eval_script import eval_one, eval_response_answer_one
...
retrieved_res = retriever.batch(questions)
retrieved_res = [[r.page_content for r in docs] for docs in retrieved_res]

# 加载重排器
logging.info("Loading as reranker")
...
for i in tqdm(range(len(questions))):
    scores = reranker.compute_score([[questions[i], retrieved_res[i][j]] for j in range(len(retrieved_res[i]))])
    # Softmax scores，需要考虑负数
    scores = [s - min(scores) for s in scores]
    scores = [s / sum(scores) for s in scores]
    scores_lst.append(scores)

    # 查看Reranker的@1，@5的准确率
    retrieval_tmp = retrieved_res[i][:]
    # 排序
    retrieval_tmp = [retrieval_tmp[idx] for idx in sorted(range(len(scores)), key=lambda x: scores[x], reverse=True)]
    # @1
    if eval_response_answer_one(retrieval_tmp[0], answers[i]):
        rerank_acc_1 += 1
        logging.info(f"Question {i}: hit @1")
    # @5
    if eval_response_answer_one(retrieval_tmp[:5], answers[i]):
        rerank_acc_5 += 1
        logging.info(f"Question {i}: hit @5")
    # @10
    if eval_response_answer_one(retrieval_tmp[:10], answers[i]):
        rerank_acc_10 += 1
        logging.info(f"Question {i}: hit @10")
    logging.info(f"scores: {scores}")
logging.info(f"Reranker @1: {rerank_acc_1 / len(questions)}")
logging.info(f"Reranker @5: {rerank_acc_5 / len(questions)}")
logging.info(f"Reranker @10: {rerank_acc_10 / len(questions)}")
...
#
# logging.info(f"model structure: {model.config}")
